...
for i in range(len(space)):
    ...
    for j in range(len(nlags)):
        ...
        for k in range(100):
          ...
          h[0], = ax.step(range(nlags[j]+1),r2, color='cyan')
        ...
        if j == 0:
            ax.legend(h, ['Synthetic', 'Historical', '95% limit'], loc='upper center')
    ...
    fig.suptitle(title + ' Space Autocorrelation at ' + site,fontsize=16)    
    fig.savefig('figures/autocorr-' + space[i] + '_' + site + '.pdf')
    fig.show()
