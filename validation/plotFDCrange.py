...
def plotFDCrange(syntheticData, histData, sites, evapIndices):
    ...
    for j in range(np.shape(histData)[1]):
        ...
        if j in evapIndices:
            ax.plot(P,np.min(F_syn,0),c='steelblue',label='Synthetic')
            ax.plot(P,np.max(F_syn,0),c='steelblue',label='Synthetic')
            ax.plot(P,np.min(F_hist,0),c='turquoise',label='Historical')
            ax.plot(P,np.max(F_hist,0),c='turquoise',label='Historical')
            ax.set_ylabel('Rate (in/day)')
        else:
            ax.semilogy(P,np.min(F_syn,0),c='steelblue',label='Synthetic')
            ax.semilogy(P,np.max(F_syn,0),c='steelblue',label='Synthetic')
            ax.semilogy(P,np.min(F_hist,0),c='turquoise',label='Historical')
            ax.semilogy(P,np.max(F_hist,0),c='turquoise',label='Historical')
            ax.set_ylabel('Flow (m$\mathregular{^3}\!$/s)')
        
        # fill between min and max to show range of FDCs
        ax.fill_between(P, np.min(F_syn,0), np.max(F_syn,0), color='steelblue')
        ax.fill_between(P, np.min(F_hist,0), np.max(F_hist,0), color='turquoise')
        ax.set_title(sites[j],fontsize=15)
    ...
    fig.legend(handles, labels, fontsize=15,loc='lower center',ncol=2, frameon=True)
    fig.text(0.5,0.1,'Probability of Exceedance',ha='center',fontsize=15)
...
histData = histData[:,[0,24]] 
syntheticData = syntheticData[:,[0,24]] 
sites = [ 'Trann','Paris']
